				...
				<paginate name="promos" v-if="promos" :list="promos" class="paginate-list margin-60" :per="4">
					<div class="row event_container" v-for="(promo,index) in paginated('promos')" :class="{ 'last': index === (paginated('promos').length - 1) }"  :key="promo.id">
						...
						<div class="col-sm-6 col-md-8 event_dets_container">
							...
							<router-link v-if="promo.promotionable_type == 'Store'" class="event_store_container" :to="'/stores/'+promo.store.slug">
							...
							<div class="text-right  col-sm-6" v-if="promo" style="padding:0">
								<router-link :to="'/promotions/'+ promo.slug" class="event_learn_more pull-left hvr-icon-wobble-horizontal">
            ...
            methods: {
                ...
                checkimageURL(image_url){
                    if (value.store != null && value.store != undefined && _.includes(value.store.image_url, 'missing')) {
                       return = "//codecloud.cdn.speedyrails.net/sites/5a6a54eb6e6f647da51e0100/image/png/1518554684072/bonniedoonlogo.png";
                    }
